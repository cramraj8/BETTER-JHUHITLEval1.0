...
public class RM3Searcherv3 extends IndexSearcher {
    double lambda;
    int expandAmount;
    int numDocs;
    long docSum;
    ...
    public RM3Searcherv3(IndexReader r, MainSearcher mainSearcher) {
        super(r);
    ...
    public RM3Searcherv3(IndexReader r, int wordExpand) {
        super(r);
    ...
    // @Override
    public TopDocs search(Query query, String req_text, ScoreDoc[] labelRelevanceTaskScoreDocs, ScoreDoc[] labelRelevanceRequestScoreDocs, int n) throws IOException, QueryNodeException {
        ...
        ScoreDoc[] newQEDocs = reRank(combinedScoreDocsArray, query, req_text);
    ...
    public ScoreDoc[] reRank(ScoreDoc[] initialResults, Query q, String req_text) throws IOException {

        // req-text query
        double reqLambda = 0.75;
        HashMap<String, Double> reqQueryCounts = new HashMap<String, Double>();
        for (String req_query : req_text.split(" ")) {
            if(reqQueryCounts.containsKey(req_query)){
                reqQueryCounts.put(req_query, reqQueryCounts.get(req_query) + 1);
            }
            else{
                reqQueryCounts.put(req_query, 1.0);
            }
        }
        int hashReqQuerySize = reqQueryCounts.size();
        for(String query: req_text.split(" ")){ // get query level probabilities
            Double count = reqQueryCounts.get(query) / hashReqQuerySize;
            reqQueryCounts.put(query, count);
        }
...
//        }
        ...
        for(int i=0; i < rankingScores.length; i ++){
            rankingScores[i] = generateScore(initialResults[i].doc, this.getIndexReader(), queryTerms, globalWordCount, reqQueryCounts);
    ...
    private float generateScore(int docID, IndexReader indexReader, ArrayList<String> queryTerms, HashMap<String, Long> counts, HashMap<String, Double> reqQueryCounts) {

        double reqLambda = 0.25;
        ...
        for(String e : queryTerms){
            ...
            if (reqQueryCounts.get(e) != null){
                reqQProb = reqQueryCounts.get(e);
            }else{
                reqQProb = 0.0;
            }
            ...
            cumProduct +=  Math.log(
                            reqLambda *
                                lambda * ( wordFreq/totalFreq ) +
                                        (1-lambda) * (globalWordProb) +
                                    (1-reqLambda) * reqQProb +
                                        1 ); //add plus one to avoid taking the log of 0. which will return nan.
