...
public class RM3SearcherHITLEnhancedv2 extends IndexSearcher {
    double lambda;
    int expandAmount;
    int numDocs;
    long docSum;
    ...
    public RM3SearcherHITLEnhancedv2(IndexReader r, MainSearcher mainSearcher) {
        super(r);
    ...
    public RM3SearcherHITLEnhancedv2(IndexReader r, int wordExpand) {
        super(r);
    ...
    // @Override
    public TopDocs search(Query query, ScoreDoc[] labelRelevanceTaskScoreDocs, ScoreDoc[] labelRelevanceRequestScoreDocs, int n) throws IOException, QueryNodeException {
        ...
        ScoreDoc[] newQEDocs = reRank(combinedScoreDocsArray, query);
    ...
    public ScoreDoc[] reRank(ScoreDoc[] initialResults, Query q) throws IOException {
    ...
    private float generateScore(int docID, IndexReader indexReader, ArrayList<String> queryTerms, HashMap<String, Long> counts) {
        ...
        for(String e : queryTerms){
            ...
            cumProduct +=  Math.log( muLambda * ( wordFreq/totalFreq ) + (1-muLambda) * (globalWordProb) + 1 ); //add plus one to avoid taking the log of 0. which will return nan.
        }
