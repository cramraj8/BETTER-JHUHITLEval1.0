...
public class RM3Searcherv1 extends IndexSearcher {
    ...
    String stopFilePath;
    List stopWordsList;
    public MainArabicSearcher mainSearcher;
    public EnglishAnalyzer englishAnalyzer;
    public EnglishStemmer englishStemmer;
    public Logger logger;
    public RM3Searcherv1(IndexReader r, Logger logger, MainArabicSearcher mainSearcher) {
        super(r);
        ...
        this.stopFilePath = mainSearcher.prop.getProperty("Arabic.stopWords.path");
    ...
    // @Override
    public TopDocs search(Query query, int n, ArbRequestBETTER betterRequest) throws IOException {

        // Step-1 : Get top-1000 retrieved documents
        TopDocs top1000Hits = super.search(query, n) ;

        // Step-5 : do RM3 mix & QE
        ScoreDoc[] newQEDocs = reRank(top1000Hits.scoreDocs, query, betterRequest);
    ...
    public ScoreDoc[] reRank(ScoreDoc[] initialResults, Query q, ArbRequestBETTER betterRequest) throws IOException {
...
//        }
    ...
    private float generateScore(int docID, IndexReader indexReader, ArrayList<String> queryTerms, HashMap<String, Long> counts) {
        ...
        long totalFreq;
        try {
            t = indexReader.getTermVector(docID, this.mainSearcher.prop.getProperty("ARB_FIELD_TOKENIZED_CONTENT"));
    ...
    private HashMap<String, Double> scoreWords(ScoreDoc[] hits, Query q, HashMap<String, Long> globalWordCount) {
        ...
        for ( int i =0;  i < this.numDocs; i ++){ //only iterate through the relevant document set not the corpus
            hit = hits[i];
            try {
                Terms t = reader.getTermVector(hit.doc, this.mainSearcher.prop.getProperty("ARB_FIELD_TOKENIZED_CONTENT"));
    ...
    private HashMap<String, Long> globalWordCount(ScoreDoc[] hits) throws IOException {
        ...
        for(int i=0; i < this.numDocs; i++){
            Terms a  = reader.getTermVector(hits[i].doc, this.mainSearcher.prop.getProperty("ARB_FIELD_TOKENIZED_CONTENT"));
