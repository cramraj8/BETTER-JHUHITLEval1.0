...
public class HITLRM3Searcher extends IndexSearcher {
    double lambda;
    int expandAmount;
    int numDocs;
    long docSum;
    ...
    String stopFilePath;
    List stopWordsList;
    public HITLMainSearcher hitlMainSearcher;
    public EnglishAnalyzer englishAnalyzer;
    public EnglishStemmer englishStemmer;
    public Logger logger;
    public HITLRM3Searcher(IndexReader r, Logger logger, HITLMainSearcher hitlMainSearcher) {
        super(r);
        this.setSimilarity(new LMDirichletSimilarity());
        this.hitlMainSearcher = hitlMainSearcher;
        this.expandAmount = hitlMainSearcher.numFeedbackTerms;
        this.numDocs = hitlMainSearcher.numFeedbackDocs;
        this.lambda = hitlMainSearcher.rm3Mix;
        this.stopFilePath = hitlMainSearcher.prop.getProperty("stopWords.path");
    ...
    public TopDocs search(Query query, ScoreDoc[] labelRelevanceTaskScoreDocs, ScoreDoc[] labelRelevanceRequestScoreDocs, int n, BETTERRequest betterRequest) throws IOException {
        ...
        List<ScoreDoc> TopNewQEDocsArrayList = newQEDocsArrayList.subList(0, this.hitlMainSearcher.numHits);
        ...
        TopDocs newHits = new TopDocs(new TotalHits(this.hitlMainSearcher.numHits, TotalHits.Relation.GREATER_THAN_OR_EQUAL_TO), reRankedNewScoreDocs);
    ...
    private String QETermsAnalyze(HashMap<String, Double> wordsMap) throws IOException {
        ...
        TokenStream stream = englishAnalyzer.tokenStream(this.hitlMainSearcher.fieldToSearch, new StringReader(QRWordsFlatString));
    ...
    public ScoreDoc[] reRank(ScoreDoc[] initialResults, Query q, BETTERRequest betterRequest) throws IOException {
        ...
        QEBuffer.append("\n");
        this.hitlMainSearcher.QEWriteFile.write(QEBuffer.toString());
...
//        }
    ...
    private float generateScore(int docID, IndexReader indexReader, ArrayList<String> queryTerms, HashMap<String, Long> counts) {
        ...
        long totalFreq;
        try {
            t = indexReader.getTermVector(docID, this.hitlMainSearcher.prop.getProperty("FIELD_BODY"));
            totalFreq = t.getSumTotalTermFreq();
        }catch (IOException e ){
            logger.severe("Exception : generateScore - HITLRM3Searcher.java : Something happened");
            return 0;
        }
        ArrayList<TermWrapperHITL> docTerms = getTerms(t, indexReader);
        ...
        for(String e : queryTerms){
            i = docTerms.indexOf(new TermWrapperHITL(e,0) ); // the query word is in the document
            if( i >= 0 ){
    ...
    private HashMap<String, Double> scoreWords(ScoreDoc[] hits, Query q, HashMap<String, Long> globalWordCount) {
        ...
        for ( int i =0;  i < this.numDocs; i ++){ //only iterate through the relevant document set not the corpus
            ...
            try {
                ...
                //double lambda = ( this.mu)/( totalFreq + this.mu);
                ArrayList<TermWrapperHITL> docTerms = getTerms(t, reader);
                double q_given_D = 1;  // double check this // pre-compute p(q|D). previous search models score is this value
                q_given_D = hit.score;

                for(TermWrapperHITL e: docTerms){ //calculate p(t|d)*p(q|d)
    ...
    public ArrayList<TermWrapperHITL> getTerms(Terms terms, IndexReader reader){
        TermsEnum a;
        TermWrapperHITL t ;
        BytesRef term ;
        long termFreq;
        ArrayList<TermWrapperHITL> output = new ArrayList<TermWrapperHITL>();
        try{
            a =terms.iterator();
            while( ( term=a.next())!= null ){
                termFreq =  a.totalTermFreq();
                t = new TermWrapperHITL(term.utf8ToString(), termFreq );
                output.add(t);
            }
        }catch (IOException e){
    ...
    private HashMap<String, Long> globalWordCount(ScoreDoc[] hits) throws IOException {
        ...
        for(int i=0; i < this.numDocs; i++){
            Terms a  = reader.getTermVector(hits[i].doc, this.hitlMainSearcher.prop.getProperty("FIELD_BODY"));
