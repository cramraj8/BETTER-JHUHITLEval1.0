...
public class HumanRM3Searcher extends IndexSearcher {
    double lambda;
    int expandAmount;
    int numDocs;
    long docSum;
    ...
    String stopFilePath;
    List stopWordsList;
    public HumanMainSearcher humanMainSearcher;
    public EnglishAnalyzer englishAnalyzer;
    public EnglishStemmer englishStemmer;
    public Logger logger;
    public HumanRM3Searcher(IndexReader r, Logger logger, HumanMainSearcher humanMainSearcher) {
        super(r);
        this.setSimilarity(new LMDirichletSimilarity());
        this.humanMainSearcher = humanMainSearcher;
        this.expandAmount = humanMainSearcher.numFeedbackTerms;
        this.numDocs = humanMainSearcher.numFeedbackDocs;
        this.lambda = humanMainSearcher.rm3Mix;
        this.stopFilePath = humanMainSearcher.prop.getProperty("stopWords.path");
    ...
    public List<String> getStopWordsList() {
        ...
        try {
            ...
            logger.severe("Exception - HITLRM3Searcher.java - getStopWordsList : Stop-words file not found : " + this.stopFilePath);
            System.exit(1);
        }
    ...
    private ScoreDoc[] mergeTaskReqScoreDocs(ScoreDoc[] labelRelevanceTaskScoreDocs, ScoreDoc[] labelRelevanceRequestScoreDocs) {
        if (labelRelevanceTaskScoreDocs.length == 0) return labelRelevanceRequestScoreDocs;
         if (labelRelevanceRequestScoreDocs.length == 0) return labelRelevanceTaskScoreDocs;
    ...
    private ScoreDoc[] mergeHumanPseudoScoreDocs(ScoreDoc[] labelRelevanceTaskRequestScoreDocs, ScoreDoc[] labelRelevanceHumanScoreDocs) {
        if (labelRelevanceTaskRequestScoreDocs.length == 0) return labelRelevanceHumanScoreDocs;
        // if (labelRelevanceHumanScoreDocs.length == 0) return labelRelevanceTaskRequestScoreDocs;

        boolean foundRelMatch;
        ArrayList<ScoreDoc> mergedNewScoreDocs = new ArrayList<ScoreDoc>();
        for (ScoreDoc labelRelevanceHumanScoreDoc : labelRelevanceHumanScoreDocs) {
            foundRelMatch = false;
            for (ScoreDoc labelRelevanceTaskRequestScoreDoc : labelRelevanceTaskRequestScoreDocs) {

                if (labelRelevanceHumanScoreDoc.doc == labelRelevanceTaskRequestScoreDoc.doc) {
                    mergedNewScoreDocs.add( new ScoreDoc(labelRelevanceTaskRequestScoreDoc.doc,
                            labelRelevanceTaskRequestScoreDoc.score) ); // this is 2.0; i am not summing up 1.0 + 2.0 TODO:
            ...
            if (!foundRelMatch) mergedNewScoreDocs.add( new ScoreDoc(labelRelevanceHumanScoreDoc.doc, labelRelevanceHumanScoreDoc.score) );
    ...
    // @Override
    public TopDocs search(Query query, ScoreDoc[] labelRelevanceTaskScoreDocs, ScoreDoc[] labelRelevanceRequestScoreDocs, ScoreDoc[] labelRelevanceHumanScoreDocs, int n, BETTERRequest betterRequest) throws IOException {
    // public TopDocs search(Query query, ScoreDoc[] labelRelevanceTaskScoreDocs, ScoreDoc[] labelRelevanceRequestScoreDocs, int n, BETTERRequest betterRequest) throws IOException {
        ...
        if (labelRelevanceHumanScoreDocs != null) {
            if (labelRelevanceHumanScoreDocs.length != 0) {
                mergedTaskReqScoreDocs = mergeHumanPseudoScoreDocs(mergedTaskReqScoreDocs, labelRelevanceHumanScoreDocs);
            }
        }

//        System.out.println("===============================================");
//        // System.out.println(mergedTaskReqScoreDocs.length);
//        for (ScoreDoc mergedTaskReqScoreDoc : mergedTaskReqScoreDocs) System.out.println(mergedTaskReqScoreDoc);
//        System.out.println("===============================================");
        ...
        ScoreDoc[] newQEDocs = reRank(combinedScoreDocsArray, query, betterRequest);
        ...
        List<ScoreDoc> TopNewQEDocsArrayList = newQEDocsArrayList.subList(0, this.humanMainSearcher.numHits);
        ...
        TopDocs newHits = new TopDocs(new TotalHits(this.humanMainSearcher.numHits, TotalHits.Relation.GREATER_THAN_OR_EQUAL_TO), reRankedNewScoreDocs);
    ...
    private String QETermsAnalyze(HashMap<String, Double> wordsMap) throws IOException {
        ...
        TokenStream stream = englishAnalyzer.tokenStream(this.humanMainSearcher.fieldToSearch, new StringReader(QRWordsFlatString));
    ...
    public ScoreDoc[] reRank(ScoreDoc[] initialResults, Query q, BETTERRequest betterRequest) throws IOException {
        ...
        QEBuffer.append("\n");
        this.humanMainSearcher.QEWriteFile.write(QEBuffer.toString());
...
//        }
    ...
    private float generateScore(int docID, IndexReader indexReader, ArrayList<String> queryTerms, HashMap<String, Long> counts) {
        ...
        long totalFreq;
        try {
            t = indexReader.getTermVector(docID, this.humanMainSearcher.prop.getProperty("FIELD_BODY"));
            totalFreq = t.getSumTotalTermFreq();
        }catch (IOException e ){
            logger.severe("Exception : generateScore - HITLRM3Searcher.java : Something happened");
            return 0;
        }
        ArrayList<TermWrapperHuman> docTerms = getTerms(t, indexReader);
        ...
        for(String e : queryTerms){
            i = docTerms.indexOf(new TermWrapperHuman(e,0) ); // the query word is in the document
            if( i >= 0 ){
    ...
    private Query expandQuery(ArrayList<String> wordCandidate, Query q) {
        ...
        try{
            ...
            logger.warning("Exception : expandQuery - HITLRM3Searcher.java : Couldn't parse returning original query");
            return q;
        }
    }
    ...
    private HashMap<String, Double> scoreWords(ScoreDoc[] hits, Query q, HashMap<String, Long> globalWordCount) {
        ...
        for ( int i =0;  i < this.numDocs; i ++){ //only iterate through the relevant document set not the corpus
            ...
            try {
                ...
                //double lambda = ( this.mu)/( totalFreq + this.mu);
                ArrayList<TermWrapperHuman> docTerms = getTerms(t, reader);
                double q_given_D = 1;  // double check this // pre-compute p(q|D). previous search models score is this value
                q_given_D = hit.score;

                for(TermWrapperHuman e: docTerms){ //calculate p(t|d)*p(q|d)
                ...
                logger.severe("Exception : scoreWords - HITLRM3Searcher.java : Something happened");
                System.exit(1);
            }
        }
    ...
    public ArrayList<TermWrapperHuman> getTerms(Terms terms, IndexReader reader){
        TermsEnum a;
        TermWrapperHuman t ;
        BytesRef term ;
        long termFreq;
        ArrayList<TermWrapperHuman> output = new ArrayList<TermWrapperHuman>();
        try{
            a =terms.iterator();
            while( ( term=a.next())!= null ){
                termFreq =  a.totalTermFreq();
                t = new TermWrapperHuman(term.utf8ToString(), termFreq );
                output.add(t);
            }
        }catch (IOException e){
            logger.severe("Exception : getTerms - HITLRM3Searcher.java : messed up with iterator in getTerms");
            System.exit(1);
        }
        return output;
    }
    ...
    private HashMap<String, Long> globalWordCount(ScoreDoc[] hits) throws IOException {
        ...
        for(int i=0; i < this.numDocs; i++){
            Terms a  = reader.getTermVector(hits[i].doc, this.humanMainSearcher.prop.getProperty("FIELD_BODY"));
