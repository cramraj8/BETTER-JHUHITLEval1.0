...
public class RM3SearcherDecryption extends IndexSearcher {
    double lambda;
    int expandAmount;
    int numDocs;
    long docSum;
    ...
    String stopFilePath;
    List stopWordsList;
    public InitialFullHITLExtractor mainSearcher;
    public EnglishAnalyzer englishAnalyzer;
    public EnglishStemmer englishStemmer;
    public Logger logger;
    public RM3SearcherDecryption(IndexReader r, Logger logger, InitialFullHITLExtractor mainSearcher) {
        super(r);
...
//        }
    ...
    private float generateScore(int docID, IndexReader indexReader, ArrayList<String> queryTerms, HashMap<String, Long> counts) {
        ...
        ArrayList<TermWrapperDecryption> docTerms = getTerms(t, indexReader);
        ...
        for(String e : queryTerms){
            i = docTerms.indexOf(new TermWrapperDecryption(e,0) ); // the query word is in the document
            if( i >= 0 ){
    ...
    private HashMap<String, Double> scoreWords(ScoreDoc[] hits, Query q, HashMap<String, Long> globalWordCount) {
        ...
        for ( int i =0;  i < this.numDocs; i ++){ //only iterate through the relevant document set not the corpus
            ...
            try {
                ...
                //double lambda = ( this.mu)/( totalFreq + this.mu);
                ArrayList<TermWrapperDecryption> docTerms = getTerms(t, reader);
                double q_given_D = 1;  // double check this // pre-compute p(q|D). previous search models score is this value
                q_given_D = hit.score;

                for(TermWrapperDecryption e: docTerms){ //calculate p(t|d)*p(q|d)
    ...
    public ArrayList<TermWrapperDecryption> getTerms(Terms terms, IndexReader reader){
        TermsEnum a;
        TermWrapperDecryption t ;
        BytesRef term ;
        long termFreq;
        ArrayList<TermWrapperDecryption> output = new ArrayList<TermWrapperDecryption>();
        try{
            a =terms.iterator();
            while( ( term=a.next())!= null ){
                termFreq =  a.totalTermFreq();
                t = new TermWrapperDecryption(term.utf8ToString(), termFreq );
                output.add(t);
            }
        }catch (IOException e){
